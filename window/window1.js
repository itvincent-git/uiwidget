...
if(top != this){
	...
	windowOpen = function(){
		...
		var iframe = new Ext.ux.ManagedIFrame({autoCreate:{id:ifid||(winId+'Ifm'),
			width:'100%',height:'100%',frameborder:"0"
	    ,scrolling:"auto",src:url}});
		
	    iframe.on('documentloaded', function(){alert(123)});
	    iframe.on('domready', function(){alert("domready")});
	    /*var iframe = {
	    tag: "iframe"
	    ,id:ifid||(winId+'Ifm')
	    ,name:ifid||(winId+'Ifm')
		...
		/*ifEl.load({
		    ...
		    //text: "Loading...",
		    timeout: 30,
		    scripts: true // Save time by avoiding RegExp execution.
		});
		var ifEl = Ext.DomHelper.append(wrapEl, iframe, true);
		delete iframe;
		...
		if(config.loadMask !== false){
			//var loadMask = new Ext.LoadMask(wrapEl,
			//	Ext.apply({msg:'加载中...',removeMask:true}, config.loadMask));
		}
		
		var h= Ext.isIE?'onreadystatechange':'onload';
        //ifEl.dom['DOMContentLoaded'] = function(){alert('DOMContentLoaded')};
        //ifEl.dom['DOMFrameContentLoaded'] = function(){alert('DOMFrameContentLoaded ')};
    	ifEl.dom[h] = ifEl.dom[h]?
        	ifEl.dom[h].createSequence(loadHandler,ifEl):
        	loadHandler.createDelegate(ifEl);
        function loadHandler(e){
	    	//var rstatus = ifEl.dom.readyState || e.type;
        	var rstatus = (e && typeof e.type !== 'undefined'?e.type:ifEl.dom.readyState );
        	//alert('rstatus = ' + rstatus);
	         switch(rstatus){
	            case 'loading':
	               break;
        ...
        */    
		win = new Ext.Window({
				...
				//ifm:ifEl,
				//loadMask:loadMask,
				draggable:config.draggable === false? false:true,//是否可拖拉
				maximizable:config.maximizable === true? true:false,//是否可以最大化
				minimizable:config.minimizable === true? true:false,//是否可以最小化
				resizable:config.resizable === false? false:true,//是否可以改变大小
		...
		win.on('close', function(thisWin){
			destroy(thisWin);
		});
		        
       	function destroy(thisWin){
       		var ifm = thisWin.body;
	        if(ifm.dom){
 	...
 	*/
  	windowClose = function(){
   	...
   	windowClose2 = function(){
   		...
   		//var closeButton = Ext.get(winId).child("div.x-tool-close");
   		//alert(Ext.type(closeButton.dom));
   		//closeButton.dom.click();
