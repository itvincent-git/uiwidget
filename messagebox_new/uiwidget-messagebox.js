	...
	*/
    ...
    ,ERROR_TITLE : '失败'
		...
		,keydown: function(e){
			...
			var key = e.which;
			if(key == 13){
				var	btns = e.data.windowEl.find("input");
				if(btns.length > 0){
					var btn0 = btns.first();				
					btn0.focus();
					btn0.click();
	    ...
	    var t = this;
	    //绑定按钮事件
	    this.windowEl.find("input").each(function(i){
      ...
      this.customCommand(args);
		...
		,setButton : function(buttons){
			...
			for(var i=0;i<buttons.length;i++)
		      ...
		      this.buttonEl.append(['<input type="button" value="',buttons[i].text,'" class="btn ',buttons[i].iconCls,'" />'].join(''));
		    }
		    else{
		    	this.buttonEl.append(['<input type="button" value="',buttons[i].text,'" class="btn" />'].join(''));
		    }
		}		
		,show : function(){
			this.getWindow().show();
	...
	$.messagebox = function(cfg){
		...
		$.uiwidget.Messagebox.instance.message(cfg);
		return $.uiwidget.Messagebox.instance;
	};
})(jQuery);	